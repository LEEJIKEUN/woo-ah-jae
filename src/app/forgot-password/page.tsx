...
export default function ForgotPasswordPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-6">
      <h1 className="mb-2 text-3xl font-bold text-slate-900">비밀번호 찾기</h1>
      <p className="mb-6 text-sm text-slate-500">가입한 이메일을 입력하면 비밀번호 재설정 링크를 발급합니다.</p>
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-2xl bg-[color:var(--surface)] p-6">
        ...
        {message ? <p className="rounded-md bg-emerald-500/10 px-3 py-2 text-sm text-emerald-600">{message}</p> : null}
        {error ? <p className="rounded-md bg-rose-500/10 px-3 py-2 text-sm text-rose-600">{error}</p> : null}
        {warning ? <p className="rounded-md bg-amber-500/10 px-3 py-2 text-sm text-amber-700">{warning}</p> : null}
        {resetUrl ? (
          <p className="rounded-md bg-slate-100/70 px-3 py-2 text-xs text-slate-600">
            개발용 재설정 링크:{" "}
            <a href={resetUrl} className="text-cyan-600 underline underline-offset-2">
              {resetUrl}
            </a>
          </p>
        ) : null}
      ...
      <Link href="/login" className="mt-4 text-sm text-slate-600 underline underline-offset-4">
        로그인으로 돌아가기
      </Link>
    </main>
  );
