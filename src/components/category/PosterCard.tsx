...
export default function PosterCard({ item, rank }: Props) {
  return (
    <Link href={`/projects/${item.id}`} className="group relative block w-[172px] shrink-0 snap-start sm:w-[200px]" aria-label={`${item.title} 상세 보기`}>
      {rank ? (
        <span className="pointer-events-none absolute -left-2 -top-2 z-20 rounded-md bg-white/70 px-2 py-0.5 text-xs font-bold text-white sm:-left-3 sm:-top-3">
          {rank}
        </span>
      ) : null}
      ...
      <motion.article
        ...
        className="relative h-[258px] overflow-hidden rounded-2xl bg-white sm:h-[300px]"
      >
        <div className="absolute inset-0" style={{ backgroundImage: gradientByTab(item.tab) }} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/45 to-transparent" />

        <div className="absolute left-2 top-2 rounded-md bg-white/25 px-2 py-0.5 text-[10px] text-white/90">
          {item.channel}
        </div>
        ...
        >
          <LikeButton
            ...
            className="inline-flex items-center gap-1 rounded-md bg-white/35 px-2 py-1 text-[11px] text-slate-900"
          />
        </motion.div>

        <div className="absolute inset-x-0 bottom-0 p-3">
          <h3 className="line-clamp-2 text-sm font-semibold leading-5 text-white">{item.title}</h3>
          <p className="mt-1 line-clamp-1 text-[11px] text-slate-600">{item.status === "open" ? "모집중" : "마감"} · {item.capacity}명</p>
        </div>
        ...
        <motion.div
          ...
          whileHover={{ opacity: 1 }}
          transition={{ duration: 0.15, ease: "easeOut" }}
          className="pointer-events-none absolute inset-0 flex flex-col justify-end bg-white/68 p-3 opacity-0"
        >
          <p className="line-clamp-1 text-xs font-semibold text-slate-900">{item.title}</p>
          <p className="mt-1 line-clamp-1 text-xs text-slate-800">{item.summary}</p>
          <p className="mt-1 line-clamp-1 text-[11px] text-slate-600">{item.status === "open" ? "모집중" : "마감"} · 좋아요 {item.likeCount}</p>
        </motion.div>
      </motion.article>
    </Link>
  );
