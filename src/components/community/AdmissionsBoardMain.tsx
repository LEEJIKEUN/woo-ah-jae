    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">독립 게시판 구조로 운영되는 입시 커뮤니티</p>
        </div>
        {canWrite && writeSlug ? (
      ...
      <form className="flex flex-nowrap items-center gap-2 overflow-x-auto rounded-lg border border-slate-200/70 bg-[color:var(--surface)] p-3">
        <input type="hidden" name="board" value={boardParam} />
        <select name="sort" defaultValue={sort} className="h-10 w-32 shrink-0 rounded-md border border-slate-200/80 bg-transparent px-2 text-sm text-slate-900">
          <option value="latest">최신순</option>
          <option value="views">조회순</option>
          <option value="likes">추천순</option>
          <option value="comments">댓글많은순</option>
        </select>
        <input name="q" defaultValue={query} placeholder="검색" className="h-10 min-w-[240px] flex-1 rounded-md border border-slate-200/80 bg-transparent px-3 text-sm text-slate-900" />
          ...
          <div className="space-y-1">
            {noticePosts.map((item) => (
              <Link key={item.id} href={`/boards/posts/${item.id}`} className="flex items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-slate-100/40">
                <span className="rounded bg-amber-400/20 px-1.5 py-0.5 text-[11px] text-amber-700">{item.isPinned ? "고정" : "공지"}</span>
                <span className="line-clamp-1 flex-1 text-slate-900">{item.title}</span>
                <span className="text-xs text-slate-500">{item.boardChannel.name}</span>
