    ...
    <div className="w-full py-6">
      <div className="mb-3 flex items-center justify-between px-4 min-[900px]:hidden">
        <button
          ...
          className="rounded-md border border-slate-200/80 bg-[color:var(--surface)] px-3 py-2 text-sm font-semibold text-slate-900"
        >
          게시판 메뉴
        </button>
      </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-[70] bg-white/60 min-[900px]:hidden" onClick={() => setOpen(false)}>
          <div
            className="h-full w-[86%] max-w-[320px] border-r border-slate-200/80 bg-[color:var(--surface)] p-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-900">게시판</p>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="rounded-md border border-slate-200/80 px-2 py-1 text-xs text-slate-800"
              >
                닫기
              </button>
            </div>
