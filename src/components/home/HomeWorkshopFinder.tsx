...
export default function HomeWorkshopFinder() {
  ...
  return (
    <section id="find" className="mx-auto max-w-[1000px] px-6 pb-40 xl:max-w-[1240px]">
      {isAdmin ? (
        <div className="mb-5 flex justify-end">
          <Link href="/course/new" className="rounded-full px-4 py-2 text-[13.5px] font-bold text-white" style={{ background: BROWN }}>＋ 강좌 개설 요청</Link>
        </div>
      ) : null}
      {/* 필터 펼치기 (기본=숨김) */}
      {!show ? (
            ...
            <thead>
              <tr className="border-b" style={{ borderColor: "#D8CFBD" }}>
                {["강좌명", "대상", "형식", "방식", "기간", "국가", "신청현황", "신청마감", "상태"].map((h) => (
                  <th key={h} className="whitespace-nowrap pb-3 pr-3 text-[14px] font-semibold" style={{ color: INK }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {results.map((r) => (
                <tr key={r.id} className="border-b" style={{ borderColor: "#EFEAE0" }}>
                  <td className="whitespace-nowrap py-5 pr-3">
                    <Link href={r.href} className="block max-w-[230px] truncate text-[14px] hover:underline" style={{ color: BODY }} title={r.name}>{r.name}</Link>
                  </td>
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>{r.target}</td>
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>{r.format}</td>
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>{r.mode}</td>
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>{r.periodLabel}</td>
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>
                  ...
                  <td className="whitespace-nowrap py-5 pr-3 text-[14px]" style={{ color: BODY }}>
                    {isAdmin ? (
                      <input type="date" value={toDateInput(r.deadline)} onChange={(e) => patchMeta(r.id, { deadline: e.target.value })} className="rounded-[8px] border bg-white px-1.5 py-1 text-[12px]" style={{ borderColor: LINE, color: INK }} />
                    ) : (
