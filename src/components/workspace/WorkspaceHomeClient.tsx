...
function ExternalLinkButton({ label, href }: { label: string; href: string | null }) {
  if (!href) {
    return <span className="rounded-md border border-slate-200 px-2 py-1 text-slate-500">{label}</span>;
  }
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="rounded-md border border-slate-200 px-2 py-1 text-slate-800 hover:border-slate-400"
    >
      {label}
    </a>
  );
...
export default function WorkspaceHomeClient(props: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-slate-200/70 bg-[color:var(--surface)] p-4">
        <h1 className="text-2xl font-bold text-slate-900">{props.projectTitle}</h1>
        <p className="mt-1 text-sm text-slate-600">{props.summary}</p>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
          <Link href={`/workspace/${props.projectId}/files`} className="rounded-md border border-slate-200 px-2 py-1 text-slate-800 hover:border-slate-400">자료실</Link>
          {props.canManage ? <Link href={`/workspace/${props.projectId}/settings`} className="rounded-md border border-slate-200 px-2 py-1 text-slate-800 hover:border-slate-400">워크스페이스 설정</Link> : null}
      ...
      <section className="space-y-3 rounded-xl border border-slate-200/70 bg-[color:var(--surface)] p-4">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-lg font-semibold text-slate-900">일정 관리</h2>
          <div className="flex items-center gap-2 text-xs text-slate-600">
            <button type="button" onClick={() => setCurrentMonth((prev) => addMonths(prev, -1))} className="rounded-md border border-slate-200 px-2 py-1 hover:border-slate-400">이전 달</button>
            <span className="min-w-[150px] text-center text-sm font-semibold tracking-[0.1em] text-slate-900">{currentMonth.getFullYear()} {currentMonth.toLocaleString("en-US", { month: "short" }).toUpperCase()}</span>
            <button type="button" onClick={() => setCurrentMonth((prev) => addMonths(prev, 1))} className="rounded-md border border-slate-200 px-2 py-1 hover:border-slate-400">다음 달</button>
          </div>
        </div>

        <div className="overflow-hidden rounded-md border border-slate-200/80">
          <div className="grid bg-white text-center text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-800" style={{ gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>
            {["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map((label, index) => (
              <div key={label} className={`py-2 ${index === 6 ? "" : "border-r border-slate-200/80"}`}>{label}</div>
            ))}
          </div>
            ...
            {dayCells.map((cell, index) => {
              ...
              return (
                <button
                  ...
                  className={`grid h-full min-h-0 w-full grid-rows-[auto_1fr] overflow-hidden border-t border-slate-200/80 p-1.5 text-left transition hover:bg-white/40 ${isLastCol ? "" : "border-r border-slate-200/80"} ${selected ? "bg-cyan-500/10 ring-1 ring-inset ring-cyan-400/60" : "bg-white/30"}`}
                >
                  <div className={`text-xs font-semibold ${cell.inCurrentMonth ? "text-slate-900" : "text-slate-500"}`}>{cell.day}</div>
                  <div className="mt-1 space-y-1 overflow-hidden">
                    {events.slice(0, 2).map((event) => (
                      <div key={event.id} className={`truncate whitespace-nowrap rounded px-1.5 py-0.5 text-[10px] ${event.done ? "bg-slate-200/70 text-slate-600 line-through" : "bg-rose-300/20 text-rose-100"}`}>
                        {event.title}
                      </div>
                    ))}
                    {events.length > 2 ? <div className="text-[10px] text-slate-500">+{events.length - 2}개</div> : null}
                  </div>
                </button>
              );
            })}
        ...
        <div className="rounded-md border border-slate-200/80 bg-white/40 p-3">
          <p className="text-xs font-semibold text-slate-600">{selectedDate.getMonth() + 1}월 {selectedDate.getDate()}일 일정</p>
            ...
            <div className="space-y-1.5">
              {selectedDateSchedules.length === 0 ? (
                ...
                selectedDateSchedules.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-2 rounded-md border border-slate-200/80 bg-white/40 px-2 py-2">
                    <label className="flex min-w-0 items-start gap-2 text-xs text-slate-800">
                      <input type="checkbox" checked={item.done} onChange={(e) => void toggleSchedule(item.id, e.target.checked)} className="mt-0.5" />
                      <span className="min-w-0">
                        <span className={item.done ? "line-through text-slate-500" : "text-slate-900"}>{item.title}</span>
                        {item.note ? <span className="mt-0.5 block text-[11px] text-slate-500">{item.note}</span> : null}
                      </span>
                    </label>
                    <button type="button" onClick={() => void removeSchedule(item.id)} className="text-[11px] text-rose-600 hover:text-rose-700">삭제</button>
                  </div>
                ))
              )}
            </div>

            <input value={scheduleTitle} onChange={(e) => setScheduleTitle(e.target.value)} className="h-8 w-full rounded-md border border-slate-200/80 bg-[color:var(--surface)] px-2 text-xs text-slate-900" placeholder="일정 또는 To-do 제목" />
            <input value={scheduleNote} onChange={(e) => setScheduleNote(e.target.value)} className="h-8 w-full rounded-md border border-slate-200/80 bg-[color:var(--surface)] px-2 text-xs text-slate-900" placeholder="메모(선택)" />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="space-y-3 rounded-xl border border-slate-200/70 bg-[color:var(--surface)] p-4">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-lg font-semibold text-slate-900">프로젝트 개요</h2>
        ...
        <section className="space-y-3 rounded-xl border border-slate-200/70 bg-[color:var(--surface)] p-4">
          ...
          <div ref={listRef} className="h-[420px] overflow-y-auto rounded-md border border-slate-200/80 bg-white/40 p-3">
            <div className="space-y-3">
              {messages.map((message) => (
                <article key={message.id} className="rounded-md border border-slate-200/80 bg-white/50 p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs text-slate-600">{message.user.school} · {message.user.grade} · {message.user.name}</p>
                  ...
                  {message.type === "FILE" && message.fileUrl ? (
                    <a href={message.fileUrl} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs text-cyan-600 hover:text-cyan-200">첨부파일 열기: {message.fileName ?? "파일"}</a>
                  ) : null}
                </article>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <label className="cursor-pointer rounded-md border border-slate-300/80 px-3 py-2 text-xs text-slate-800 hover:border-slate-300">
                파일 첨부
                <input
                  type="file"
                  className="hidden"
