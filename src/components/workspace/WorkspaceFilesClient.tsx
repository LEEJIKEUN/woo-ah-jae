  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <label className="cursor-pointer rounded-md border border-slate-300 px-3 py-2 text-xs text-slate-900 hover:border-slate-300">
          파일 업로드
          <input
            type="file"
            className="hidden"
      ...
      {uploading ? <p className="text-xs text-slate-500">업로드 중...</p> : null}

      <div className="space-y-2">
        {items.length === 0 ? (
          <p className="text-sm text-slate-500">업로드된 파일이 없습니다.</p>
        ) : (
          items.map((item) => (
            <article key={item.id} className="rounded-xl border border-slate-200/70 bg-[color:var(--surface)] p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-slate-900">{item.fileName}</p>
                  <p className="text-xs text-slate-500">{item.mimeType} · {(item.size / 1024 / 1024).toFixed(2)}MB</p>
                ...
                <a href={item.fileUrl} target="_blank" rel="noreferrer" className="rounded-md border border-slate-300 px-2 py-1 text-xs text-slate-900 hover:border-slate-300">
                  열기
                </a>
              </div>
            </article>
